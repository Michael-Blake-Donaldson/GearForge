...
export default function PracticeScreen() {
  ...
  const quizCandidates = useMemo(() => {
    ...
    const lessonIds = lessons.filter((lesson) => lesson.regionId === regionId).map((lesson) => lesson.id);
  ...
  const weakQuizzes = quizzes.filter((quiz) =>
    quiz.questions.some((question) => incorrectQuestionIds.includes(question.id))
  );

  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      <Text style={styles.title}>Practice Arena</Text>
      <Text style={styles.subtitle}>Reinforce weak topics and sharpen quiz speed with targeted drills.</Text>

      <View style={styles.card}>
        <Text style={styles.cardTitle}>Weak Topic Review</Text>
        <Text style={styles.cardBody}>{incorrectQuestionIds.length} weak questions identified so far.</Text>

        {weakQuizzes.length === 0 ? (
          <Text style={styles.emptyState}>No weak quizzes yet. Complete more lessons to build a review set.</Text>
        ) : (
          weakQuizzes.slice(0, 4).map((quiz) => (
            <Pressable key={quiz.id} style={styles.actionButton} onPress={() => router.push(`/quiz/${quiz.id}`)}>
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.cardBody}>Choose a region focus or run all-region random drills.</Text>
        ...
        <View style={styles.filters}>
          ...
          {regions.slice(0, 3).map((region) => (
            <Pressable
              ...
              <Text style={styles.filterText}>{region.name.replace(' Vehicles', '')}</Text>
            </Pressable>
          ))}
        </View>
