  ...
  region.units.flatMap((unit, unitIndex) => {
    ...
    return lessonModuleTemplates.map((module, lessonIndex) => {
      ...
      return {
        ...
        realWorldContext:
          `In service bays, technicians use this model to avoid guesswork, reduce comeback repairs, and make confident decisions under time and budget constraints.`,
        symptoms:
          `Typical symptoms include degraded response, unstable behavior under load transitions, warning indicators, and intermittent performance drift during sustained operation.`,
        keyTakeaway:
          `${unit.title} decisions are strongest when diagnosis follows system evidence instead of assumptions.`,
        quizId,
        order: lessonOrder,
